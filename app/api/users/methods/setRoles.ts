...
Meteor.methods({
    'set.users.updateRoles': async function ({ role, users, removeRole }: UpdateUserRolesInput) {
        check(users, [String]);
        ...
        const res: GetUserRolesResult = await Meteor.callAsync('get.users.roles', data);
