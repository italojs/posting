...
async function main() {
    ...
    console.log(`[demo-03] Delta -> added: ${added.length}, changed: ${changed.length}, removed: ${removed.length}`);
    if (added.length)
        console.log('[demo-03] sample added:', added.slice(0, 3).map((x) => x.title || keyOf(x)));
    if (changed.length)
        console.log('[demo-03] sample changed:', changed.slice(0, 3).map((x) => x.title || keyOf(x)));
    if (removed.length)
        console.log('[demo-03] sample removed keys:', removed.slice(0, 3));
    // 4) Save new cache
