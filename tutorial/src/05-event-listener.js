...
async function main() {
    ...
    while (!stop && cycles < maxCycles) {
        ...
        try {
            ...
            for (const it of delta.changed)
                console.log('[demo-05][changed]', prettyItem(it));
            for (const k of delta.removed)
                console.log('[demo-05][removed]', k);
            console.log(`[demo-05] cycle ${cycles}/${maxCycles} -> added:${delta.added.length} changed:${delta.changed.length} removed:${delta.removed.length}`);
            previous = fresh;
        }
        catch (err) {
